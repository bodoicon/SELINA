...
function StatCard({ icon, title, value }) {
  return (
    <Card className="h-full">
      <CardContent className="grid min-h-[98px] grid-cols-[56px_minmax(0,1fr)] items-center gap-4 px-6 py-5">
        <div className="flex h-14 w-14 items-center justify-center self-center overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 p-1 text-slate-700">{icon}</div>
        <div className="flex min-h-[56px] flex-col justify-center self-center">
...
export default function HoaHoiGameCanvasApp() {
  ...
  useEffect(() => {
    ...
    if (activeTab === "adminpanel" && isSuperAdmin && !statCardAssetsLoaded) loadStatCardAssetsData().catch((e) => setPageMessage(e.message));
