    ...
    <Card className="rounded-[28px]">
      ...
      <CardContent className="grid gap-4 xl:grid-cols-[340px_1fr]">
        <div className="space-y-4">
          ...
          <div className="space-y-2"><Label>Icon bình</Label><Input value={artVaseForm.iconUrl} onChange={(e) => setArtVaseForm((prev) => ({ ...prev, iconUrl: e.target.value }))} placeholder="https://..." /><div className="flex items-center gap-3 rounded-2xl border bg-slate-50 p-3">{artVaseForm.iconUrl ? <div className="h-14 w-14 overflow-hidden rounded-2xl border bg-white"><img src={artVaseForm.iconUrl} alt={artVaseForm.name || "Icon bình hoa"} className="h-full w-full object-cover" /></div> : <div className="flex h-14 w-14 items-center justify-center rounded-2xl border bg-white text-slate-400"><PlaceholderFlowerIcon size="sm" /></div>}<label className="block flex-1"><input type="file" accept="image/*" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) uploadArtVaseIcon(file); e.target.value = ""; }} /><span className="inline-flex h-10 w-full cursor-pointer items-center justify-center rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50">{uploadingArtVaseIcon ? "Đang tải icon..." : "Tải icon bình hoa"}</span></label></div>{artVaseMessage ? <div className="rounded-2xl border bg-slate-50 p-3 text-sm text-slate-700">{artVaseMessage}</div> : null}</div>
